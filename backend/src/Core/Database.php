...
class Database
{
    ...
    ): ?array
    {
        ...
        $query = <<<SQL
        SELECT uid, firstname, lastname, username, email, password
        FROM users
        WHERE $conditions
        SQL;
     ...
     * @throws \FranklinEkemezie\ComputerVisionAfrica\Exceptions\DBException
    ...
    ): int|null
    {
        ...
        try {
            ...
            if ((int) $e->getCode() === self::ERROR_DUPLICATE_ENTRY)
                throw new DBException("Duplicate entry error: {$e->getMessage()}", (int) $e->getCode());
